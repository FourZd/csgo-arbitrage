...
async def run_parser(cursor, conn, bots):
    ...
    for i in range(bots_amount):
        ...
        tasks.append(asyncio.create_task(run_period(bots[i], period, game)))
    await asyncio.gather(*tasks)
    await run_period(cursor, conn)
        ...
        proxy = [
            {
                ...
                'proxy_username': bot['proxy_user'],
                'proxy_password': bot['proxy_password']
            } for bot in bots if bot['id'] == account['id']
        ][0]
        
        profile = await get_firefox_profile(proxy['proxy_host'], proxy['proxy_username'], proxy['proxy_password'])
        cookies = await login(account['steam_login'], account['steam_password'], account['email'], account['email_password'], profile)
...
async def run():
    ...
    # Get bots credentials for authentication with proxy
    bots = get_buff_credentials(cursor)
